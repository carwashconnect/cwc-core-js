...
describe("Promises", function () {
    ...
    it("wait() should wait some time before completing", async function (done) {
        try {
            const waitTime = 2000;
    ...
    it("loadBalancer() resolve basic promises", async function (done) {
        try {
            ...
            console.error(e);
            expect(true).toEqual(false, "Encountered error while testing load balancer.");
            done();
        }
    });

    it("loadBalancer() to halt if timeout is going to be exceeded", async function (done) {
        try {
    ...
    it("loadBalancer() auto balancer creates square sets", async function (done) {
        try {
            ...
            //4x4
            let responses1 = await Promises.loadBalancer(timedPromises, { timeout: (WAIT_TIME * 2) + 20, verbose: false })
            expect(responses1[0].result).toEqual(1);
            expect(responses1[0].executionTime).toBeGreaterThanOrEqual(WAIT_TIME);
            expect(responses1[1].result).toEqual(2);
            expect(responses1[1].executionTime).toBeGreaterThanOrEqual(WAIT_TIME);
    ...
    it("loadBalancer() shuffle should not impact result order", async function (done) {
        try {
